...
const deepseekReuqest = axios.create({  // 创建一个axios实例
...
deepseekReuqest.interceptors.request.use((config) => {  // 添加请求拦截器
  return config;  // 直接返回配置对象，不做任何修改
})

deepseekReuqest.interceptors.response.use((response) => {  // 添加响应拦截器
